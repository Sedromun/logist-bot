...
warehouse_router.message.middleware(AccessMiddleware(role="warehouse"))

@warehouse_router.callback_query(lambda c: c.data.startswith("accept|"))
async def accept_shipment(query: CallbackQuery):
    ...
    await bot.send_message(chat_id=admin.id, text=f"Склад принял вывоз:\n {shipment._asdict()}")

    await query.message.edit_text(text="Вы успешно приняли вывоз!")
    await query.answer()


@warehouse_router.callback_query(lambda c: c.data.startswith("dismiss|"))
async def dismiss_shipment(query: CallbackQuery):
    ...
    await bot.send_message(chat_id=admin.id, text=f"Склад отказался от вывоза:\n {shipment._asdict()}")
